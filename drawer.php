...
$zappit_auth_token = "eyJhbGciOiJSUzI1NiIsImtpZCI6IjNmZDA3MmRmYTM4MDU2NzlmMTZmZTQxNzM4YzJhM2FkM2Y5MGIyMTQiLCJ0eXAiOiJKV1QifQ..jqRtVJZYdGKW1I7df2rRwVvX26v9teBDkaq93faLJhUcCmk2mEFIrDGxvs2gDlJxDZt267GY36Lg4C-Z4XzfMaZM1ZpJhCvuemSAUQXMe9mK3VmS5iU_47DVu6EV7ap83J4R9BhYqyOaoC1csqtpyCuoFzeTF3-Y8yXtWGXA0lbufIvExlsMGWOx6hxEerm8ctpV8Shay2t-ldl99mfn5iFAYCedno6dWY41-vsapRObiy1-WiZBDxFN9dhFI2g7kJWwegXkd9b-k7vc2yQ1nRw7_Yuwg3XasG6ljg6zpaHmIlXQQB_W8uck-mL80zS6G3SrvAGjPmvR18jeg63HnA";
...
function my_plugin_display_posts()
{
    ...
    <div class="p-6 bg-white rounded-lg shadow-lg w-full max-w-full mx-auto mt-8">
        ...
        <h2 class="text-2xl font-semibold text-gray-800 mb-5 inline-flex">
            <img src="<?php echo esc_url(get_option('zappit_ai_company_logo_url')); ?>" alt="Logo"
                style="max-height: 50px; vertical-align: middle; margin-right: 10px;">
        ...
        <div class="mb-6 border-b border-gray-300">
            <nav class="flex justify-start space-x-6 w-full" role="tablist">
                ...
                <!-- Content Tab Button -->
                <button
                    class="tab-button text-[#667085] font-medium px-6 py-3 hover:text-blue-600 focus:text-blue-600 focus:outline-none focus:border-b-2 focus:border-blue-600 transition-colors duration-200 w-auto"
                    role="tab" aria-selected="true" data-tab="content">Content</button>

                <!-- Technical Tab Button -->
                <button
                    class="tab-button text-[#667085] font-medium px-6 py-3 hover:text-blue-600 focus:text-blue-600 focus:outline-none focus:border-b-2 focus:border-blue-600 transition-colors duration-200 w-auto"
                    role="tab" aria-selected="false" data-tab="technical">Technical</button>

                <!-- Page Fixes Tab Button -->
                <button
                    class="tab-button text-[#667085] font-medium px-6 py-3 hover:text-blue-600 focus:text-blue-600 focus:outline-none focus:border-b-2 focus:border-blue-600 transition-colors duration-200 w-auto"
                    role="tab" aria-selected="false" data-tab="page-fixes">Page Fixes</button>
...
function display_technical_table($meta_key)
{
    ...
    <div class="overflow-x-auto">
        ...
        <div id="pagination-controls" class="flex items-center gap-4 p-4 bg-gray-100 rounded-lg shadow-md">
            <!-- Previous Button -->
            <button id="prev-btn"
                class="bg-blue-500 text-white font-semibold py-1 px-2 rounded-lg hover:bg-blue-600 transition-colors duration-300 disabled:bg-gray-300 disabled:cursor-not-allowed text-sm"
                disabled>
                Previous
            </button>
        ...
        <script>
            class ServerSidePagination {
                ...
                renderData(items) {
                    ...
                    // Render each item
                    <?php
                    // Retrieve items from wherever they're stored (e.g., database query)
                    $items = get_action_items(); // Replace with your actual method of fetching items
                    ...
                    if (!empty($items)) {
                        foreach ($items as $item) {
                            ...
                            $item_desc = isset($item['desc']) ? sanitize_text_field($item['desc']) : '';
                            $item_page_id = isset($item['page_id']) ? sanitize_text_field($item['page_id']) : '';
                            $item_assets_folder = isset($item['plugin_assets_folder']) ? esc_url($item['plugin_assets_folder']) : '';

                            // Create slug safely
                            $item_desc_slug = create_slug_($item_desc);
                            ?>
                                < tr class="bg-white border-t border-gray-200 hover:bg-[#F3F7FD] transition-colors duration-300" >
                    <td class="px-6 py-4 text-sm text-gray-800"><?php echo esc_html($item_id); ?></td>            
                    <td class="px-6 py-4 text-sm text-gray-800"><?php echo esc_html($item_desc); ?></td>
                    <td class="px-6 py-4">
                        <div class="flex justify-center space-x-2">
                            <div id="technicalDrawer-<?php echo esc_attr($item_page_id . '-' . $item_desc_slug); ?>" 
                                 class="fixed top-0 left-0 w-full h-full bg-gray-900 bg-opacity-50 flex items-center justify-center opacity-0 pointer-events-none transition-opacity duration-300">
                                <div class="bg-white rounded-lg shadow-lg w-full max-w-5xl p-6 relative lg:ml-40">
                                    <button id="closeTechnicalDrawer-<?php echo esc_attr($item_page_id . '-' . $item_desc_slug); ?>" 
                                            class="absolute top-4 left-4 text-gray-600 hover:text-gray-800 flex items-center ml-2 mt-1">
                                        <img src="<?php echo esc_url($item_assets_folder); ?>/images/arrow_back.png" alt="" class="w-5 h-5">
                                    </button>
                                    <div class="px-9 py-4">
                                        <h2 class="text-xl font-bold"><?php echo esc_html($item_desc); ?></h2>
                                        <p><?php echo esc_html($item_desc); ?></p>
                                    </div>
                                </div>
                            </div>
                            <button id="openTechnicalDrawer-<?php echo esc_attr($item_page_id . '-' . $item_desc_slug); ?>" 
                                    class="border border-blue-500 text-blue-500 px-4 py-2 rounded-lg text-sm hover:bg-blue-100 transition-all duration-300">
                                View Details
                            </button>
                        </div>
                    </td>
                        ...
                        // Optional: Display a message if no items exist
        ...
        <table class="min-w-full bg-[#F9FAFB] rounded-lg border border-gray-200">
            ...
            <tbody>
                ...
                foreach ($pageActions["pageFixes"] as $index => $action) {
                    ...
                    $post_status = 'draft'; // Default initial state
                    $action_button_label = $post_status === 'publish' ? 'Unpublish' : 'Publish';
                    $action_button_color = $post_status === 'publish'
                        ? 'border border-gray-300 text-black px-4 py-2 rounded-md text-sm hover:bg-gray-100 hover:text-black hover:border-gray-400'
                        : 'bg-[#1E6FF1] text-white px-7 py-2 rounded-md text-sm hover:bg-blue-600';
                    ...
                    <tr class="bg-white border-t border-gray-200 hover:bg-[#F3F7FD] transition-colors duration-300">
                        <td class="px-6 py-4 text-sm text-[#1D1D1D]"><?php echo esc_html($action_desc); ?></td>
                        <td class="px-6 py-4 text-center">
                            <div class="flex justify-center space-x-4">
                                <button class="<?php echo esc_attr($action_button_color); ?>"
                                    data-post-id="<?php echo esc_attr($action_page_id); ?>"
                                    data-action="<?php echo esc_attr($action_button_label); ?>" onclick="toggleButton(this)">
                                    <?php echo esc_html($action_button_label); ?>
                                </button>
                                <button data-drawer="<?php echo $drawer_id; ?>"
                                    class="open-drawer-btn border border-blue-500 text-blue-500 px-4 py-2 rounded-lg text-sm hover:bg-blue-100 transition-all duration-300">
                                    View Details
                                </button>
                            </div>
                        </td>
                    ...
                    <!-- Drawer -->
                    <div id="<?php echo $drawer_id; ?>"
                        class="fixed top-0 left-0 w-full h-full bg-gray-900 bg-opacity-50 flex flex-col items-center justify-center opacity-0 pointer-events-none transition-opacity duration-300">
                        <div
                            class="bg-white rounded-lg shadow-lg w-full max-w-5xl p-6 relative lg:ml-[8rem] h-[90vh] mt-5 overflow-auto">
                            <!-- Back Button -->
                            <button data-drawer="<?php echo $drawer_id; ?>"
                                class="close-drawer-btn absolute top-4 left-4 text-gray-600 hover:text-gray-800 flex items-center ml-2 mt-1">
                                ...
                                <div class="space-y-2">
                                    <div class="flex items-center">
                                        ...
                                        <a href="<?php echo $action_url; ?>"
                                            class="text-blue-500 underline text-sm"><?php echo $action_url; ?></a>
                                    </div>
                                    <div class="flex items-center">
                                        <strong class="w-32 text-[#717171] text-sm font-medium">Type:</strong>
                                        <span
                                            class="bg-blue-100 text-blue-500 px-2 rounded-2xl text-sm flex items-center gap-2 font-medium">
                            ...
                            <div class="border-2 rounded-xl m-4">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="flex items-center justify-between p-3 rounded-lg">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <code
                                                    class="block text-black border border-gray-300 rounded p-1 py-3 text-[0.5rem] font-mono bg-[#F7F9FB]">
                                            &lt;meta name="description" content="<span class="text-blue-600">Welcome to our website. We offer great services and products.</span>"&gt; 
                                        </code>
                                            </div>
                                            <!-- Arrow -->
                                            <div class="mx-3 ">
                                                <img src="<?php echo $plugin_assets_folder ?>/images/arrow_right.png"
                                                    alt="Arrow" class="w-6 h-2 mt-4">
                                            </div>
                                            <!-- After -->
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <div class="flex items-center justify-between">
                                                    <code id="metaTagContent"
                                                        class="flex text-black border border-gray-300 rounded p-1 py-3 text-[0.5rem] font-mono bg-[#F7F9FB]">
                                <span>
                                        ...
                                        <div>
                                            ...
                                            &lt;title&gt;<span class="text-blue-600">Home</span>&lt;/title&gt;
                                                ...
                                                <div class="flex-1 ">
                                                    ...
                                                    <code id="titleTagContent"
                                                        class="flex items-center text-black border border-gray-300 rounded p-1 py-3 text-[0.5rem] font-mono bg-[#F7F9FB]">
                                ...
                                <span class="flex items-center justify-end h-5 w-full pr-1 cursor-pointer">
                                    <img id="copyTitleTag" src="<?php echo $plugin_assets_folder ?>/images/copy.png" alt="Copy">
                                                ...
                                                <!-- Toaster -->
                                                <div id="toaster"
                                                    class="hidden fixed top-4 right-4 bg-blue-600 text-white text-sm rounded px-4 py-2 shadow-lg">
                                                    Copied to clipboard!
                                                </div>
